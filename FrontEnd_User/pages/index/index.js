...
Page({
  ...
  getRoute() {
    // 起点
    let { latitude, longitude, markers, markerId, city,startData, textData } = this.data;
    let { name, desc } = startData;
    var name2=textData.name;
    if (!markers.length) return;
    // 终点
    let { latitude: latitude2, longitude: longitude2 } = markers[markerId];
    //console.log(name,name2);
    let url = `/pages/confirm/confirm?longitude=${longitude}&latitude=${latitude}&longitude2=${longitude2}&latitude2=${latitude2}&city=${city}&name=${name}&name2=${name2}&type=${"0"}`;
    wx.navigateTo({ url });
  },
  //结束点搜索跳转(废弃函数)
